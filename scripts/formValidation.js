...
cardnumberInput.addEventListener('invalid', () => {
  if(cardnumberInput.value === '') {
    ...
    cardnumberInput.setCustomValidity('Credit cards have 16 digits. Try again!');
  }
});
...
csvInput.addEventListener('invalid', () => {
  if(csvInput.value === '') {
    ...
    csvInput.setCustomValidity('Usernames can only contain upper and lowercase letters. Try again!');
  }
});
...
expiryDateInput.addEventListener('invalid', () => {
  if(expiryDateInput.value === '') {
    ...
    expiryDateInput.setCustomValidity('Expiry dates must be after the current date. Try again!');
  }
});
